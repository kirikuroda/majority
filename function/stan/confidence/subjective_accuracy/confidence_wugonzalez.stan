...
transformed parameters {
  ...
  vector[N] q;
  for (i in 1:N) {
    q[i] = pow(p_hat[i], alpha[id[i]]) / pow(
      pow(p_hat[i], alpha[id[i]]) + pow(1 - p_hat[i], alpha[id[i]]), beta[id[i]]
    );
  }
}
...
model {
  ...
  if (alpha[4] < 1 || alpha[15] < 1 || alpha[44] < 1 || alpha[51] < 1 || alpha[55] < 1) {
  ...
  mean_alpha ~ cauchy(0, 1);
  mean_beta ~ cauchy(0, 1);
  mean_tau ~ cauchy(0, 5);
  sd_alpha ~ student_t(10, 0, 1);
  sd_beta ~ student_t(10, 0, 1);
