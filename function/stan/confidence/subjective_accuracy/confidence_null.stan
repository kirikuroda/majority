//
// Author: Kiri Kuroda
// Subjective accuracy in each trial (Null model)
//
...
model {
  ...
  for (i in 1:N) {
    predictor[i] = tau[id[i]] * 
      (pow(scaled_reward[i], rho[id[i]]) * p_hat[i] - pow(scaled_500, rho[id[i]]));
  }
...
generated quantities {
  ...
  vector[4] y_pred[N_subj];
  {
    ...
    for (i in 1:N) {
      // Generate data
      predictor[i] = tau[id[i]] * (pow(scaled_reward[i], rho[id[i]]) * p_hat[i] -
      ...
      log_lik[i] = bernoulli_logit_lpmf(choice[i] | 
        tau[id[i]] * (
          pow(scaled_reward[i], rho[id[i]]) * p_hat[i] -
          pow(scaled_500, rho[id[i]])
        )
      );
    }
