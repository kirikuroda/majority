//
// Author: Kiri Kuroda
// Subjective accuracy in each trial (Goldstein-Einhorn function)
//
...
transformed parameters {
  ...
  vector[N] q;
  for (i in 1:N) {
    q[i] = alpha[id[i]] * pow(p_hat[i], beta[id[i]]) /
      (alpha[id[i]] * pow(p_hat[i], beta[id[i]]) + pow(1 - p_hat[i], beta[id[i]]));
  }
}
