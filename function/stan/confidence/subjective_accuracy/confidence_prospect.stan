//
// Author: Kiri Kuroda
// Subjective accuracy in each trial (Prospect theory by Kahneman & Tversky)
//
...
transformed parameters {
  ...
  for (i in 1:N) {
    q[i] = pow(p_hat[i], alpha[id[i]]) / pow(
      pow(p_hat[i], alpha[id[i]]) + pow(1 - p_hat[i], alpha[id[i]]), 1 / alpha[id[i]]
    );
  }
}
...
model {
  ...
  alpha ~ lognormal(mean_alpha, sd_alpha);
