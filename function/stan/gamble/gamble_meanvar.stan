...
// Here we assume mean-variance utility function and 
// logit choice model.
//
...
data {
  ...
  vector<lower=0>[N] reward; // Reward magnitude
  vector<lower=0, upper=1>[N] prob; // Reward probability
  real<lower=0> scale; // Scaling parameter of reward
}
...
parameters {
  ...
  real<lower=0> sd_tau;
  vector[N_subj] alpha; // Individual risk preference
...
model {
  ...
  vector[N] predictor;
  for (i in 1:N) {
    predictor[i] = tau[id[i]] * (scaled_reward[i] * prob[i] +
      alpha[id[i]] * pow(scaled_reward[i], 2) * prob[i] * (1 - prob[i]) -
      scaled_500);
  }
...
generated quantities {
  ...
  for (i in 1:N) {
    ...
    y_pred[id[i]] = y_pred[id[i]] +
      bernoulli_logit_rng(
        tau[id[i]] * (scaled_reward[i] * prob[i] +
        alpha[id[i]] * pow(scaled_reward[i], 2) * prob[i] * (1 - prob[i]) -
        scaled_500)
      );
